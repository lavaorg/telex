...
func TestAddFields(t *testing.T) {
	metrics := make(chan telex.Metric, 10)
	defer close(metrics)
	...
	tp := testm.Type()
	require.Equal(t, telex.Counter, tp)
}
...
func (tm *TestMetricMaker) MakeMetric(metric telex.Metric) telex.Metric {
	return metric
}
