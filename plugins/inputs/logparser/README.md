...
The Telex grok parser uses a slightly modified version of logstash "grok"
...
Telex has many of its own [built-in patterns](./grok/patterns/influx-patterns),
...
#### Parsing Telex log file
We can use logparser to convert the log lines generated by Telex in metrics.

To do this we need to configure Telex to write logs to a file.
This could be done using the ``agent.logfile`` parameter or configuring syslog.
```toml
[agent]
  logfile = "/var/log/telex/telex.log"
```

Logparser configuration:
```toml
[[inputs.logparser]]
  files = ["/var/log/telex/telex.log"]
...
Example log lines:
```
2018-06-14T06:41:35Z I! Starting Telex v1.6.4
...
Generated metrics:
```
telegraf_log,host=somehostname,level=I msg="Starting Telex v1.6.4" 1528958495000000000
...
- https://www.influxdata.com/telex-correlate-log-metrics-data-performance-bottlenecks/
