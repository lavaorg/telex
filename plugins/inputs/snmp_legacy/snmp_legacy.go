...
func (h *Host) SNMPGet(acc telex.Accumulator, initNode Node) error {
	// Get snmp client
	snmpClient, err := h.GetSNMPClient()
	if err != nil {
		return err
...
func (h *Host) SNMPBulk(acc telex.Accumulator, initNode Node) error {
	// Get snmp client
	snmpClient, err := h.GetSNMPClient()
	if err != nil {
		return err
...
func init() {
	inputs.Add("snmp_legacy", func() telex.Input {
		return &Snmp{}
	})
}
